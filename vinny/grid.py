...
class MVGrid:
    ...
    @staticmethod
    def _preprocess_image(image: Image.Image, height: int, width: int) -> Image.Image:
    ...
    def override_grid(self, images: List[Image.Image], overrides: Dict[str, str], grid_path: str):
        (w, h) = images[0].size
        print(f"Overriding first {len(overrides)} images, based on size: w={w} h={h}")

        side_to_idx = {"front": 0, "left": 1, "back": 2, "right": 3}
        for side, img in overrides.items():
            print(f"PreProcessing {img}...")
            image = Image.open(img)
            images[side_to_idx[side]] = self._preprocess_image(image, h, w)
